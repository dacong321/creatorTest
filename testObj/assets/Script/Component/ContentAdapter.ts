...
export default class NewClass extends cc.Component {
    ...
    FixSceneResolution(){
        ...
        //-- getCanvasSize  canvas 组件， 随时获得设备屏幕的实际分辨率。  并可以对场景中所有渲染元素进行适当的缩放（正确缩放的前提是 首先确定canvas组件所在的节点适配整个屏幕的大小）
        ...
        //-- 根据canvas获得的设备实际分辨率 和自己的设计蓝本的设计分辨率，来动态调整Canvas所在的node节点的大小。
        let handle = setInterval(
            function name() {
                if(window.gg.utils.isNull(self.node)){
                ...
                console.log(cS);
                console.log(srS);
                //如果canvas 比较宽（即宽高比大于设计分辨率）则拉升node宽度
